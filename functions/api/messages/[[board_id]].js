 ...
 */
async function handleGetRequest(boardId, db) {
    try {
        const sql = `
            SELECT
 ...
 */
async function handlePostRequest(request, boardId, db) {
    try {
        ...
        const messageInsertResult = await db.prepare('INSERT INTO board_messages (board_id, content, created_at, has_attachment) VALUES (?, ?, ?, ?)').bind(boardId, content, now, hasAttachment ? 1 : 0).run();
        const messageId = messageInsertResult.meta.last_row_id;
        if (!messageId) {
            return new Response('Failed to create message.', { status: 500 });
        }
        if (hasAttachment) {
            await db.prepare('INSERT INTO board_attachment (message_id, r2_key, filename, uploaded_at) VALUES (?, ?, ?, ?)').bind(messageId, attachment.r2Key, attachment.filename, now).run();
        }
